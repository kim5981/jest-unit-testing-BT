...
function trimProperties(obj) {
 return Object.keys(obj).reduce((newObj, val) => {
    newObj[val] = obj[val].trim()
    return newObj
  }, {})
}
 ...
 * @param {object} obj - an object with properties that are strings { ex: " example " }
 * @returns {object} - the same object with strings trimmed // mutating the object itself
 *
 * EXAMPLE
...
function findLargestInteger(integers) {
  ...
  for(let i=0; i < integers.length; i++){
    integers[i].integer > largestNumber 
...
class Car {
   ...
   */
  refuel(gallons) {
    const refuellableGals =  this.tankSize - this.tank
    if( gallons <= refuellableGals ){
